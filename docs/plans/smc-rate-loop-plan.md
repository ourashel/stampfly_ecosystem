...
#### 7.30 一時中断（ユーザー指示「区切って」、2026-09-12）
...
- 姿勢**推定**（`roll_est`/`euler.x`）は真の姿勢に対し実測で約40ms遅延・
  0.71倍減衰する——速度推定（約10ms・1.09倍、ほぼ無視できる）とは
  対照的な、無視できない大きさの実在する遅延（§7.30続報4、ユーザーの
  「むだ時間かなぁ」という直感がきっかけで発見）
- レートループのプラント同定誤差、および上記の姿勢推定遅延・減衰の
  両方を組み込んだ場合でも、**単一軸(roll)の線形/準線形PIDカスケード
  モデルでは観測された約1.73秒周期の持続振動を再現できない**——
  較正値ベースのレートプラント（§7.30続報2）、SILS実測レートプラント
  （§7.30続報3）、実測姿勢推定遅延を追加したもの（§7.30続報4）の
  **3通りの独立したモデル改良試行が、いずれも棄却**という結果になった

**次にSILS本体（Python近似モデルではなく）で直接調査すべき候補**
（優先順位はまだ未確定、次セッション開始時にユーザーと相談）:
1. 横軸間結合（yaw-rollクロスカップリング、ミキサ配分の非対称性）
2. 真のMuJoCo剛体動力学が持つ、単一スカラー慣性モデルでは捉えられない
   要素
3. ESKF推定器の姿勢推定遅延（§7.30続報4で実測・数値化済み）自体が
   なぜ生じているか（フィルタ設計・ゲイン等）の根本調査——今回は
   「値を実測してモデルに足す」までで、遅延の発生機序自体は未調査
