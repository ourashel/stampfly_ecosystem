...
**残る候補**（優先順位未確定、次回ユーザーと相談）:
1. 真のMuJoCo剛体動力学が持つ、単一スカラー慣性モデル・単一軸切り出しでは
   捉えられない要素（横軸間結合が無いと確認された今、これがどの程度残る
   候補かは要検討——慣性テンソル自体は非対角成分があっても、今回の
   シナリオでは観測可能な結合が生じていないため、優先度は下がった可能性）
2. ESKF推定器の姿勢推定遅延（§7.30続報4で実測・数値化済み）自体が
   なぜ生じているか（フィルタ設計・共分散ゲイン等）の根本調査——今回は
   「値を実測してモデルに足す」までで、遅延の発生機序自体は未調査
3. 単一軸カスケードモデル自体の妥当性を疑う——4回の独立した改良試行
   （レートプラント×2、姿勢推定遅延、軸間結合）が全て失敗したことから、
   問題は「モデルの特定パラメータ」ではなく「モデルの構造」そのものに
   ある可能性（例: 離散化誤差の蓄積、アンチワインドアップの相互作用等、
   まだ検証していないメカニズム）
