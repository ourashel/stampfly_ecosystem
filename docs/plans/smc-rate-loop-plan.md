...
#### 7.30 一時中断（2026-09-12、§7.30続報7後の更新版）
...
- ESKFの**accel-comp α-βトラッカー**の加速度状態（`a_kin_ned_`）が
  観測周波数で大きな遅れ・増幅（-70°/-336ms、3.60倍）を示すことを発見し、
  簡略化モデルで初めて「持続的・成長的な振動」を再現できた（§7.30続報6）
- **`eskf_core.cpp`への実診断計装で確認**: `a_kin_ned_.y`の実測周期は
  **約1.66秒**——観測された持続振動の周期（1.73-1.76秒）と**誤差3-5%で
  一致**。近似モデルではなく実測同士の比較であり、`roll_meas`のピーク
  付近で`a_kin_y`が急激にスパイクするという同期パターンも確認した
  （§7.30続報7）——**本調査で最も有力な証拠**

**結論・現在地**: ESKFのaccel-comp機構（フロー速度→`a_kin_ned_`推定→
姿勢補正、という閉ループ）が、観測された持続振動と強く関連していると
いう実測に基づく証拠が得られた。ただし`a_kin_ned_`の振動が「原因」か
「結果」か（既に振動しているroll推定に単に追従しているだけか、
このトラッカー自体が閉ループを不安定化させているか）は、今回のログ
だけでは完全には切り分けられていない——両者が同じ周波数で相互に強め
合う共振関係にある可能性が高い。

**次の一手（未着手、次回最優先）**: `eskf.accel_comp.enable=false`で
SILSを再実行し、この機構を無効化した場合に持続振動が消失・変化するか
を直接確認する——因果関係を確定させる最も直接的な検証。

**優先度が下がった候補**（accel-comp機構の発見により）:
1. 真のMuJoCo剛体動力学が持つ、単一スカラー慣性モデルでは捉えられない要素
2. 単一軸カスケードモデル自体の構造的妥当性
