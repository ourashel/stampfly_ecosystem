...
- [x] **重要な発見3（最新）**: 速度ループ補償にも破綻点があり、想定との差が
      ~12ms超で転倒級に発散する——「速度ループなら常に安全」ではない
- [ ] **次のステップ（実機データ収集）**: 実機の無駄時間がバッテリー電圧・飛行時間・
      個体差でどの程度変動するかを把握する（`sf sils sysid-gate`相当の手法を実機ログに
      適用、または`rate_sysid.py`等の既存同定ツールで複数条件を比較）。破綻しきい値
      （8〜20msの間）に対して十分なマージンが確認できるまで、実機投入は保留
- [ ] 実機投入する場合も、速度ループ補償のみ（`smc.velx/vely.delay_comp_ms`）を対象とし、
      レートループ補償（`smc.roll/pitch/yaw.delay_comp_ms`）は既定0のまま封印を維持
      （ミスマッチ時の被害がより大きいため）
- [ ] しきい値そのものをより正確に特定する追加SILS（8-20ms間を細かくスイープ）は未実施
