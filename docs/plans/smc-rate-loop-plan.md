...
#### 7.30 一時中断（2026-09-12、§7.30続報6後の更新版）
...
- **横軸間結合（roll→pitch/yaw）はSILS上で完全にゼロと確認——原因では
  ない**（§7.30続報5）
- ESKFの**accel-comp α-βトラッカー**（フロー速度→運動加速度推定
  `a_kin_ned_`→姿勢補正、という閉ループ）の加速度状態が観測周波数で
  大きな遅れ・増幅（-70°/-336ms、3.60倍）を示すことを発見し、これを
  簡略化してモデルに組み込んだところ、**5回目の独立した試行で初めて
  「単発の減衰」ではなく「持続的・成長的な振動」が出現**した
  （周期約2.5秒、`att_corr_gain`≈2.7-2.8付近で減衰↔成長が転移）。
  ただし周期は観測値1.73秒と完全には一致せず、`att_corr_gain`は実際の
  カルマンゲインの単純な定数近似（実測ではない）——**有力だが未確定の
  手がかり**（§7.30続報6）

**最有力候補**: ESKFのaccel-comp機構（§7.30続報6）——次に進むなら、
Python近似のさらなる調整ではなく、**実際の`eskf_core.cpp`に診断計装を
追加し、`a_kin_ned_`・イノベーション・実際のカルマンゲイン（または
その効果）を直接観測する**のが、この機構を確定させる最も確実な方法
（§7.17以来のこの調査全体の教訓: Python近似より実測が早く確実）。

**その他の残る候補**（優先度は上記より低い）:
1. 真のMuJoCo剛体動力学が持つ、単一スカラー慣性モデル・単一軸切り出しでは
   捉えられない要素（横軸間結合が無いと確認された今、優先度は下がった）
2. 単一軸カスケードモデル自体の構造的妥当性（accel-comp機構の発見で
   説明できる見込みが立ったため、優先度は下がった）

姿勢/レート/速度/yaw PIDゲイン自体の変更は本計画全体を通じて未実施・
未提案のまま（診断・解析のみ）。診断計装（`pid_controller.cpp`の
`posdiag`ログ、箇所A/B/C）は残置——次の調査でも流用できる。
