...
**実機投入: 中止（2026-09-11、`docs/plans/smc-rate-loop-plan.md` §7.28）** ——
モード遷移時のSTA速度ループ未リセットを修正（§7.27）した上で再飛行したが
クラッシュ（バッテリー脱落）。ユーザー判断により本appの実機投入は中止し、
**開発はSILSのみで継続する**。以下のビルド・書き込みコマンドはSILS検証
目的でのみ使用し、実機へは書き込まないこと。

```bash
sf app build smc_pos_sta
sf app flash smc_pos_sta -m   # 実機投入は中止（§7.28）。SILS検証以外での実行は不可
```
...
| **実機投入: 中止（2026-09-11、§7.28）** | モード遷移時のSTA速度ループ未リセットを修正（§7.27）し再飛行したがクラッシュ（バッテリー脱落）。修正後も同じ「モード切替直後」のタイミングで転倒しており、より根深い問題の可能性——ユーザー判断により実機投入は中止、以降はSILSのみで開発を継続する |
